 ...
 */
@Aspect
public abstract class AbsAopNSJsonHandler {

    private static final Logger opLogger = LoggerUtil.getOpLog();
    private static Logger logger = LoggerFactory.getLogger(AbsAopNSJsonHandler.class);

    @Around("execution(* com.ns..action..*(..)) " +
            "&& !@annotation(com.ns.common.util.annotation.IgnoreAopNSJsonHandler)")
